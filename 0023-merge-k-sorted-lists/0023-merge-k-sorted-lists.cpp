// Companies in which its asked : VMWare, Amazon, Uber, Google, Twitter, LinkedIn, Airbnb, Facebook, Microsoft, IXL

class Solution {
public:
    ListNode* mergeTwoLists(ListNode* l1, ListNode* l2) {   // O(m+n)
        // Base cases
        if(l1 == NULL)    // or if(!l1)
            return l2;
        if(l2 == NULL)    // or if(!l2)
